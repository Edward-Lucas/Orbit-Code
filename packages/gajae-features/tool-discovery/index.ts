/**
 * Tool Discovery System for Orbit Code
 *
 * Ported from gajae-code, provides BM25-based tool search.
...
// ─── Raw tool interface (generic, no external dependency) ────────────────────
...
export function isMCPBridgeTool(tool: {
  name: string;
...
function buildSearchDocument(
  tool: DiscoverableTool,
): DiscoverableToolSearchDocument {
...
/**
 * Convert a raw tool into a DiscoverableTool descriptor.
 */
export function getDiscoverableTool(
  tool: RawTool,
  overrides?: { source?: DiscoverableToolSource; summary?: string },
): DiscoverableTool | null {
  const source: DiscoverableToolSource =
    overrides?.source ?? (isMCPToolName(tool.name) ? "mcp" : "builtin");
  const rawSummary = overrides?.summary ?? tool.summary;
  ...
  return {
    ...
    schemaKeys: getSchemaPropertyKeys(tool.parameters),
  };
}

/** Collect all DiscoverableTools from a tool iterable. */
export function collectDiscoverableTools(
  ...
  for (const tool of tools) {
    ...
    const meta = getDiscoverableTool(tool, {
      source: options?.source,
      summary,
    });
    if (meta) discoverable.push(meta);
  }
  return discoverable;
}
...
export function formatDiscoverableToolServerSummary(
  server: DiscoverableToolServerSummary,
): string {
...
export function selectDiscoverableToolNamesByServer(
  ...
  return Array.from(tools)
    .filter(
      (tool) =>
        tool.serverName !== undefined && serverNames.has(tool.serverName),
    )
    .map((tool) => tool.name);
}
...
export function summarizeDiscoverableTools(
  ...
  for (const tool of tools) {
    ...
    serverToolCounts.set(
      tool.serverName,
      (serverToolCounts.get(tool.serverName) ?? 0) + 1,
    );
  }
  const servers = Array.from(serverToolCounts.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([name, toolCount]) => ({ name, toolCount }));
  return { servers, toolCount: tools.length };
}

export function buildDiscoverableToolSearchIndex(
  tools: Iterable<DiscoverableTool>,
  ...
  return index.documents
    .map((document) => {
      ...
      for (const [token, queryTermCount] of queryTermCounts) {
        ...
        const normalization =
          BM25_K1 *
          (1 - BM25_B + BM25_B * (document.length / index.averageLength));
        score +=
          queryTermCount *
          idf *
          ((termFrequency * (BM25_K1 + 1)) / (termFrequency + normalization) +
            BM25_DELTA);
      }
